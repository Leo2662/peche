...
npm test           # 154 unit tests over the scoring engine
npm run check      # both
```
...
| File | Purpose |
| --- | --- |
| `public/index.html` | `lang="fr"`, title, description, canonical, Open Graph, `theme-color` |
...
BassScore is a **single-route client-side app**: `/` is the only path the site
answers on. Listing anything else — `/dunkerque`, `/marseille` — would hand
Google soft-404s for pages that do not exist, which is worse than a short
sitemap. `ROUTES` in the generator is the single place to add entries if real
...
It tells Google the site exists and when it changed. It does **not** make a
client-rendered app rank: the served HTML has a title, a description and
nothing else, because the score only exists after JavaScript has run and the
forecast has come back. Google does render JS, but a page whose content is a
number that changes hourly has little to index.

If organic search matters, the lever is not the sitemap — it is giving the app
real URLs (a page per spot, server-rendered with its tide table and conditions).
That is a different product, and the architecture is ready for it: `Spot` is
already threaded through every layer.
...
└── ../public/              index.html template, sitemap.xml, robots.txt
...
154 tests that need no mocking framework.
...
154 tests, no mocking framework — the engine is pure, so the tests are just
