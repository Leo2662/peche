...
The website — the landing page and the guides — is a separate build, and has
its own dev server:

```bash
npm run dev        # astro dev, on :4321, with hot reload
```
...
npm test           # unit tests over the scoring engine, and over the built site
npm run check      # typecheck, build the site, then test
```

`npm run check` builds the site first because the web tests read `dist/` — what
is actually served — rather than the sources it is composed from.
...
```bash
npm run build:web     # builds the site, exports the app, merges both into dist/
npm run dev           # the site alone, with hot reload, on :4321
```
...
| `scripts/merge-app.mjs` | puts the app at `/app/`, then checks the result |

`src/` is the React Native app and Astro never sees it; `site/` is the website
and Metro never sees it. The one thing they share is `scripts/analytics.mjs`.
...
That used to be two hand-written lists in two files — `GUIDE_PAGES` in
`build-site.mjs` and `ROUTES` in `generate-sitemap.mjs` — plus a build-time
check whose entire job was to notice when someone updated one and forgot the
...
Astro writes its pages where they belong. Expo exports the app to the root of
its own output, so `scripts/merge-app.mjs` runs last and folds the two together:

```
dist/index.html          the landing page      (Astro)
dist/peche-bar-<x>/      a guide               (Astro)
dist/sitemap.xml         built from the routes (Astro)
dist/app/index.html      the app               (Expo, was dist-expo/index.html)
...
relatively, if the app is not `noindex`, if a registered guide produced no page,
or if a guide's canonical points anywhere but its own path. A silent failure
here would publish the app at `/` under the landing page's meta tags — the exact
mistake worth catching in CI rather than in Search Console.
...
`framework: null` matters more than it looks. Vercel would otherwise detect
Astro and run `astro build` on its own, which produces a perfectly good site
with **no app at `/app/`** — and no error to show for it. Only
`npm run build:web` runs all three steps. The file is checked in so the setting
travels with the repo rather than living in a dashboard someone set once and
never revisited.
...
Both are the vendors' own snippets, at the top of the `<head>` on every page.
They live in two places, because two builds produce those pages:
`site/components/Analytics.astro` for the site, and `public/index.html` — Expo's
template — for the app. Each vendor keys on the URL, so `/` and `/app/` separate
on their own with no extra configuration.

The site's copy used to be seven copies, one pasted into each document. That is
what made a build-time check necessary rather than merely prudent.
...
`clarity.ms` loader, the `async` flag and the project id. `merge-app.mjs`
checks the built pages against that list and fails naming whichever part went
missing. Expo re-serialises the app's HTML during export, and Astro minifies the
site's — a page that quietly loses a tag in either would otherwise only surface
as a hole in the reports weeks later. An id still set to its placeholder fails
the build too — a tag that ships, looks fine and reports to nobody is the
failure that already happened once here.

`tests/analytics.test.ts` pins the rest: both snippets complete on every page,
...
These are props on `BaseLayout.astro`, set by `site/pages/index.astro` — the
indexed page.
...
├── data/                   guides.mjs (the route table), sitemap.mjs
└── public/                 robots, manifest, share card and icons

public/index.html           the app's HTML template, read by Expo
```
...
247 tests, no mocking framework — the engine is pure, so the tests are just
tables of inputs and expected outputs. The web tests read `dist/`, so run
`npm run check` (which builds the site first) rather than `npm test` alone:
...
- web: the sitemap is well-formed and absolute, the file served at
  `/sitemap.xml` matches a fresh builder run, every route it announces has a
  page built behind it, and robots.txt, the canonical link and og:url all agree
...
- guides: each one claims its own path and asks to be indexed, carries exactly
  one h1 naming its place, names its own coast more than any sibling's, links
  into the tool and to every sibling but never to itself, and ships a
  breadcrumb, an `Article` about its own place, and an FAQ whose every marked-up
...
- analytics: every page carries the complete GA4 and Clarity snippets — every
